...
def get_plex_token():
    ...
    # Adding the more professional headers and regional spoofing (NYC)
    headers = {
        ...
        'X-Plex-Device': 'Linux',
        'X-Forwarded-For': '85.254.181.50', # NYC Spoofing
...
def generate_files():
    ...
    # Unified headers used for both channel and guide fetching
    headers = {
        'X-Plex-Token': token, 
        'Accept': 'application/json',
    ...
    with open("plex.m3u8", "w") as f:
        ...
        for ch in channels:
            # Skip DRM channels (they won't play anyway)
    ...
    print("Fetching guide data (this may take a moment)...")
    try:
        # 6-12 hours is the sweet spot. Too much data can cause timeouts.
        ...
        for g_ch in grid_data:
            ...
            for prog in g_ch.get("Program", []):
                # Ensure start and duration exist
                if not prog.get("start") or not prog.get("duration"): continue
                
                start_ts = int(prog.get("start"))
                stop_ts = start_ts + int(prog.get("duration"))
                
                start_xml = datetime.fromtimestamp(start_ts).strftime('%Y%m%d%H%M%S +0000')
                stop_xml = datetime.fromtimestamp(stop_ts).strftime('%Y%m%d%H%M%S +0000')
        ...
        print(f"Successfully added {program_count} programs to the guide.")
    ...
    # Python 3.9+ supports indent for pretty-printing
    try:
    ...
    print("Generation complete: plex.m3u8 and plex_guide.xml saved.")
